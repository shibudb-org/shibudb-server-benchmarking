...
def build_parser() -> argparse.ArgumentParser:
    ...
    # Server metadata (for reproducible published reports)
    ap.add_argument("--server-version", default="", help="ShibuDB server release/tag (e.g. v1.2.3)")
    ap.add_argument("--server-commit", default="", help="ShibuDB server git commit SHA")
    ap.add_argument("--server-info-file", default="",
                    help="JSON file with server hardware facts (CPU, RAM, disk, network)")
    # Suite selection
    ...
    ap.add_argument("--client-parallelism", choices=["process", "thread"], default="process",
                    help="how the load is driven: 'process' (default) forks one OS process per "
                         "worker so each has its own GIL and the server's real concurrency is "
                         "measured; 'thread' is the old GIL-bound thread pool (kept for comparison "
...
def main():
    ...
    if args.server_info_file:
        load_server_info_file(args.server_info_file)  # fail fast on bad JSON
